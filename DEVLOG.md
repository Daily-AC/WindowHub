...
**问题**：点击嵌入应用自己的关闭按钮（如飞书右上角的 ✕）可能导致应用冻结。

**原因**：

- 嵌入后窗口被设置为 `WS_CHILD` 子窗口模式
- 应用内部的关闭逻辑是为独立窗口设计的，在子窗口模式下行为不可预测

**用户指南**：

> ⚠️ 请使用 **WindowHub 标签页上的关闭按钮**（小 ✕）或快捷键 `Ctrl+W` 来关闭窗口，而不是点击嵌入应用自己的关闭按钮。

---

## 🚀 Roadmap

### v0.2: 工作区功能 (Workspaces) ✅ 已完成

- [x] 保存当前会话（记录嵌入窗口的 EXE 路径）
- [x] 一键恢复工作区（批量启动并自动嵌入）
- [x] 工作区管理 UI
- [x] 数据持久化 (`%APPDATA%\WindowHub\workspaces.json`)

### v0.3: 分屏布局 (Split View) - 计划中
